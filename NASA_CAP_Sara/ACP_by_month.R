...
for (m in months) {
  ...
  pdf(sprintf("ACP/by_month/all_variables/pca_all_var_%s.pdf", m), height = 10, width = 10, pointsize = 15)
  print(
...
for (m in months) {
  ...
  pdf(sprintf("ACP/by_month/behav/pca_behav_var_%s.pdf", m), height = 10, width = 10, pointsize = 15)
  print(
    fviz_pca_ind(res.pca, label="none") +
      geom_point(shape = 21, aes(fill = as.factor(data$pol)), col = "black", size = 2, ) +
      scale_fill_manual("Polynya", values = pal, labels = polynya_info_dict) +
...
## Behavioral variables with environmental qualitative supp var---------------
months = 3:8
...
for (m in months) {
  ...
  pdf(sprintf("ACP/by_month/behav/pca_behav_var_inout_%s.pdf", month.name[m]), height = 10, width = 10, pointsize = 15)
  print(
    fviz_pca_ind(res.pca, label="none") +
      geom_point(shape = 21, aes(fill = as.factor(data$pol)), col = "black", size = 2) +
      scale_fill_manual("Polynya", values = pal, labels = polynya_info_dict) +
