dives_outside <- readRDS("behavioural_data/dive_metrics_bottime_speed_interp_hunttime_bathy_zone_mode_pol_inpol_ctd_9") %>%
  filter(pol == 0 & !is.na(id_ctd)) %>%
  mutate(month = month(DE_DATE)) %>%
  filter(month >= 6 & month <= 8)

hist(dives_outside$MAX_DEP)

id_ctd_dives <- dives_outside %>%
  pull(id_ctd) %>%
  unique() %>%
  na.omit()
