...
## ---------------------------
## Paths
path_map <- "~/Dropbox/data/outputs_Marthe_2023/time_spent_pixels_std/"
## ---------------------------
## Library
...
## Import raster
file_ns <- "output_data/nseals_pixels/tif/RES_CESM_LR_nseals_sum_pixel_months.tif"
r_ns <- raster::brick(file_ns) #____read raster stack

file_ts <- "output_data/time_spent_pixels/tif/RES_CESM_LR_residence_time_month.tif"
r_ts <- raster::brick(file_ts) #____read raster stack
type <- "SUM" # "SUM" | "MEAN" | "REL" | ""
rt_type <- subset(r_ts, grep(type, names(r_ts)))

base.name <- sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(file_ns))
